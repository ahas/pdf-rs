...
pub fn calculate_points_for_circle<P: Into<Pt>>(
    ...
    let (radius, offset_x, offset_y) =
        (radius.into(), offset_x.into(), offset_y.into());
    let radius = radius.0;
...
#[inline]
pub fn calculate_text_size<S, F>(
    text: S,
    font: &Registered<F>,
    font_size: f64,
) -> (Pt, Pt)
    ...
    F: Borrow<Font>,
{
    ...
    let collection =
        rusttype::FontCollection::from_bytes(&*face_direct_ref.font_bytes).unwrap();
    let font = collection
        .clone()
        .into_font()
    ...
    let width = font
        .layout(text.as_str(), scale, rusttype::point(0.0, 0.0))
